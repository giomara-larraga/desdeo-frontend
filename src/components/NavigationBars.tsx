...
export const NavigationBars = ({
  ...
  const drawableSteps = Array.from(range(0, allSteps + 1)); // how many steps will be drawn
  ...
  const scaleY = useCallback(() => {
    return minOrMax.map((_, i) => {
      return scaleLinear().domain([plotHeight, 0]).range([nadir[i], ideal[i]]);
    });
  }, [dimensions]);
  ...
  const scaleX = useCallback(() => {
    return minOrMax.map(() => {
      return scaleLinear()
        .domain([0, dimensions.chartWidth])
        .range([1, allSteps]);
    });
  }, [dimensions]);
  ...
  const yAxises = useCallback(() => {
    return minOrMax.map((_, i) => {
      return axisLeft(yAxis()[i]).ticks(5);
    });
  }, [yAxis]);
  ...
  const xAxises = useCallback(() => {
    return objNames.map((_, i) => {
      return axisBottom(xAxis()[i]).tickSize(0).tickValues([]);
    });
  }, [xAxis]);
  ...
  useEffect(() => {
    if (!selection) {
      ...
      const upper_bound_color = "0000FF";
      const lower_bound_color = "C00000";
      ...
      objNames.map((_, index) => {
        ...
        enter
          ...
          .on("dblclick", function (e: any) {
            // problem now is that e.x is not in plot coordinates..
            let t = pointer(e)[0];
            console.log("pointer coord", t);
            let step = Math.floor(scaleX()[index](t));
            console.log(step);
            if (step < 1 || step > 100) {
              // do nothing
              console.log("error with step location");
            } else {
              handleGoBack(step);
            }
          });
        ...
        uppLabels
          ...
          .attr("transform", (d) => {
            return `translate( ${xAxis()[index](ulen) - 30} ${
              yAxis()[index](d) + offset * index - 5
            } )`;
          })
          .attr("font-size", "12px");
        ...
        lowLabels
          ...
          .attr("transform", (d) => {
            if (step < 4) {
              return `translate( ${xAxis()[index](llen) - 30} ${
                yAxis()[index](d) + offset * index + 20
              } )`;
            } else {
              return `translate( ${xAxis()[index](llen) - 30} ${
                yAxis()[index](d) + offset * index + 10
              } )`;
            }
          })
          .attr("font-size", "12px");
